...
// Convert included/not included to unicode icons
function convertToUnicode(value: string): string {
  if (!value) return value;
  const normalized = value.toLowerCase().trim();

  if (normalized === "included") return "✓"; // U+2713
  if (normalized === "not included") return "✕"; // U+2715
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
        ...
        if (attr.name) {
          attr.levels.forEach((level: string, idx: number) => {
            if (level) {
              ...
              const unicodeLevel = convertToUnicode(level);
      ...
      await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/Attributes!A:F:clear`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
